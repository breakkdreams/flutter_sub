      ...
      if(secret_open == '1'){
        ...
        lock_data.then((params){
          request('sendMsg', formData: {'data':params}).then((val) {
            ...
            var unlock = delock(result['data']);
            unlock.then((unlock_data){
              var data = json.decode(unlock_data);
              if (data['code'] == 200) {
                toast(data['message']);
              }
            });
      ...
      lock_data.then((params){
        request('forget_pay_password_api', formData: {'data':params}).then((val) {
          ...
          unlock.then((unlock_data){
            var data = json.decode(unlock_data);
            if(data['code'] == 200){
              Application.router.navigateTo(context,Routes.forgetPayNextPasswordPage+"?mobile="+mobile+"&mobile_code="+mController.text);
            }
            toast(data['message']);
          });
        });
